...
contract CertificateV1 {
    ...
    function nListAdmin() public view onlyOwner returns (uint) {
    ...
    function updatePerson(uint id, string memory name, string memory email, string memory birthday, address addr) public onlyAdmin {
        uint index = _getIndexPersonById(id);
    ...
    function _getIndexPersonById(uint id) internal view returns(uint) {
    ...
    function nListPerson() public view onlyAdmin returns (uint) {
        return _persons.length;
    }

    function getListPerson() public view onlyAdmin returns (Person[] memory) {
        return _persons;
    }
    ...
    // Certificate
    function createCertificate(string memory name, string memory link) public onlyAdmin {
    ...
    function _getIndexCertificateById(uint id) internal view onlyAdmin returns (uint) {
    ...
    function nListCertificate() public view onlyAdmin returns (uint) {
        return _certificates.length;
    }

    function getAllCertificate() public view onlyAdmin returns (Certificate[] memory) {
        require(_certificates.length > 0);
        return _certificates;
    }

    // Certificate for person
    function addCertificateOfPerson(uint personId, string memory nameCertificate, string memory linkCertificate) 
    public onlyAdmin isPersonExist(personId) {
        Certificate memory _newCertificate = Certificate(++_certificateIdCount, nameCertificate, linkCertificate);
        _certificates.push(_newCertificate);

        _mpCertificateOfPersons[personId].push(_newCertificate);

        _mpCertificateIdToPersonId[_newCertificate.id] = personId;
    }

    function updateCertificate(uint certificateId, string memory nameCertificate, string memory linkCertificate) 
    public onlyAdmin {
        ...
        if(personId > 0) {
            for(uint i = 0; i < _mpCertificateOfPersons[personId].length; i++) {
                if(_mpCertificateOfPersons[personId][i].id == certificateId) {
                    _mpCertificateOfPersons[personId][i] = _certificates[index];
                    return;
                }
            }
        }
    ...
    function removeCertificate(uint certificateId) public onlyAdmin {
        uint index = _getIndexCertificateById(certificateId);
    ...
    function tranferCertificateToPerson(uint certificateId, uint personId) 
    public onlyOwner isPersonExist(personId) checkCertificateExistOfPerson(certificateId) {
    ...
    function tranferManyCertificateToPerson(uint[] memory certificateIds, uint personId) 
    public onlyOwner isPersonExist(personId) {
